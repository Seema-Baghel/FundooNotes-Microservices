...
import com.bridgelabz.fundoonotes.services.UserService;
...
public class UserServiceImplementation implements UserService {
	...
	@Override
	public ResponseEntity<Response> getUser(String token) {
		long id = tokenGenerator.parseJwtToken(token);
		UserModel userId = repository.findById(id);
		if (userId != null) {
			return ResponseEntity.status(HttpStatus.OK).body(new Response("User:", Util.OK_RESPONSE_CODE, userId));
		}
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Response(Util.BAD_REQUEST_RESPONSE_CODE, "No user found"));
	}
